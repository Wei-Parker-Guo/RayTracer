...
RefracMat::RefracMat(aiMaterial* mat) {
	this->use_reflectivity = true;
	this->use_refractivity = true;
	//get base color
	aiColor3D base_color(0.5f, 0.5f, 0.5f); //default diffuse set to gray
	mat->Get(AI_MATKEY_COLOR_DIFFUSE, base_color);
	aicolor_to_vec3(this->base_c, base_color);
	...
	aiColor3D transp_color(1.0f, 1.0f, 1.0f); //default to total transparency
	mat->Get(AI_MATKEY_COLOR_TRANSPARENT, transp_color);
	aicolor_to_vec3(this->transp_c, transp_color);
	//get refractivity
	this->refractivity = 1.333f; //default to the refractive constant of water
	mat->Get(AI_MATKEY_REFRACTI, this->refractivity);
}

void RefracMat::apply_shade(vec3 r, const vec3 p, const vec3 e, const vec3 norm, std::vector<Light*> lights, shadow_ray_rec& sh_rec) {
	//zero the result
	vec3_zero(r);
	//figure out viewing vector
	vec3 view;
